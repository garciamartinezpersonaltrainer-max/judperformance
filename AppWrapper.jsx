...
// ─── Auth screens colors ──────────────────────────────────────────────────────
const C = {
  bg: "#0A0B0F", card: "#12141A", card2: "#1A1D26",
  border: "#252836", orange: "#FF6B35", orangeLight: "#FF8C5A",
  text: "#FFFFFF", textMuted: "#8891A4", textDim: "#5A6275",
  green: "#2ECC8E", red: "#FF4757",
}
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ display: 'flex', background: C.card, borderRadius: 14, padding: 4, marginBottom: 20, border: `1px solid ${C.border}` }}>
          {[['login', 'Iniciar sesión'], ['register', 'Registrarse']].map(([m, label]) => (
            <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }} style={{ flex: 1, padding: '10px', background: mode === m ? C.card2 : 'transparent', color: mode === m ? C.text : C.textMuted, border: mode === m ? `1px solid ${C.border}` : 'none', borderRadius: 11, cursor: 'pointer', fontWeight: 700, fontSize: 13 }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {/* Error / Success */}
        {error && (
          <div style={{ background: C.red + '22', border: `1px solid ${C.red}44`, borderRadius: 12, padding: '10px 14px', marginBottom: 12, color: C.red, fontSize: 13 }}>
            ⚠️ {error}
          </div>
        )}
        {success && (
          <div style={{ background: C.green + '22', border: `1px solid ${C.green}44`, borderRadius: 12, padding: '10px 14px', marginBottom: 12, color: C.green, fontSize: 13 }}>
            ✅ {success}
          </div>
        )}

        {/* Submit */}
        <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', background: loading ? C.border : `linear-gradient(135deg, ${C.orange}, ${C.orangeLight})`, border: 'none', borderRadius: 16, padding: '15px', color: '#fff', fontWeight: 800, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
          {loading ? '⏳ Cargando...' : mode === 'login' ? 'Iniciar Sesión' : 'Crear cuenta'}
        </button>

        <p style={{ color: C.textDim, fontSize: 11, textAlign: 'center', marginTop: 20, lineHeight: 1.6 }}>
          Al registrarte aceptas los términos de uso de JUD Performance.
        </p>
      </div>
    </div>
  )
...
export default function AppWrapper() {
  ...
  // Logged in — pass user to main App
